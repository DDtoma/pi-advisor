#!/usr/bin/env node
/**
 * Docs-freshness CI gate (implementation-plan Step 9):
...
const docVersion = /pi 版本:\s*([\d.]+)/.exec(apiDoc)?.[1] ?? /pi ([\d.]+)/.exec(apiDoc)?.[1];
let realVersion;
try {
...
else if (!realVersion) ok(`pi not on PATH — skipping version comparison (doc says ${docVersion})`);
else if (docVersion !== realVersion) fail(`api-verification.md says pi ${docVersion}, installed ${realVersion}`);
else ok(`pi version ${docVersion} matches`);

// (c) ADR numbering sequential
const adrs = [...readFileSync(join(root, "docs/design-decisions.md"), "utf8").matchAll(/^## ADR-(\d+)/gm)]
	.map((m) => Number(m[1]))
	.sort((a, b) => a - b);
if (adrs.length === 0) {
	...
	for (let i = 0; i < adrs.length; i++) {
		if (adrs[i] !== i + 1) {
			fail(`ADR numbering gap: expected ADR-${String(i + 1).padStart(3, "0")}, got ADR-${String(adrs[i]).padStart(3, "0")}`);
			break;
		}
	}
	if (failures === 0 || adrs.every((n, i) => n === i + 1)) ok(`ADR-001..ADR-${String(adrs.at(-1)).padStart(3, "0")} sequential`);
}
