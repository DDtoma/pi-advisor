...
### 1.1 事件订阅 `pi.on(event, handler)`(types.d.ts:2–31)
...
`PI_DIR/dist/core/extensions/runner.js:588`,在 `async emit(...)`(l.579)内部:
...
| `registerFlag(name: string, options: ...): void` / `getFlag<T>(name: string): T \| undefined` | types.d.ts:45,51 | `--advisor-model` 之类 CLI flag(可选功能) |
| `exec(command: string, args: string[], options?: ExecOptions): Promise<ExecResult>` | types.d.ts:81 | 找 `WATCHDOG.yml`、读文件以外的场景(如 git root 探测) |
| `events: EventBus` | types.d.ts:165 | runtime 内部事件总线(测试钩子用) |

---

## 2. 事件 payload 形状

### 2.1 `TurnEndEvent`(types.d.ts:556)
...
### 2.2 `BeforeAgentStartEventResult`(types.d.ts:806)
...
| `complete<TApi>(model: Model<TApi>, context: Context, options?: ModelsApiStreamOptions<TApi>): Promise<AssistantMessage>` | **★ advisor 引擎**。pi 内部完成 auth 解析、baseURL、请求、SSE 解析,返回完整 AssistantMessage(toolCall 已解析好) |
...
### 5.1 `Context`(l.377)—— `complete()` 的入参
...
| 类型 | 位置 | 关键字段 |
|---|---|---|
| `UserMessage` | l.292 | `{ role: "user", content: string \| (TextContent\|ImageContent)[], timestamp }` |
| `AssistantMessage` | l.297 | `{ role: "assistant", content: (TextContent \| ThinkingContent \| ToolCall)[], api, stopReason, usage, ... }` |
| `ToolResultMessage` | — | `{ role: "toolResult", toolCallId, toolName, content, isError, timestamp }` |
| `ToolCall` | l.246 | `{ type: "toolCall", id, name, arguments: Record<string, any>, thoughtSignature?, namespace? }` |

### 5.3 思考等级

```ts
// types.d.ts:212
ProviderStreamOptions.reasoning?: ThinkingLevel;   // "off"|"minimal"|"low"|"medium"|"high"|"xhigh"|"max"...
...
`ModelsApiStreamOptions<TApi> = ApiStreamOptions<TApi> & ModelsRequestTransforms`(models.d.ts:45)—— 完整 options 类型。

### 5.4 工具定义格式

`Tool = { name, description, parameters: TSchema }`(typebox)。advise 工具:
...
## 6. ExtensionContext(types.d.ts:191–256)
...
⚠️ 但注意:从 `extensions/index.ts` import `../src/advisor/runtime.ts` 时,路径跨越包根 —— 需要实测确认 jiti 允许(oh-my-pi 同源结构,理论上没问题;**实现第一步先做 spike 验证**,见 implementation-plan.md 第 0 步)。
